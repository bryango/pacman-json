...
complete -c pacjump -l no-reverse -d 'Do not provide reverse dependencies information'
complete -c pacjump -l optional -d '`--recurse` installed optional dependencies as well'
complete -c pacjump -l summary -d '`--recurse` dependencies, but only prints package names and versions; implies `--no-reverse` and `--plain`'
complete -c pacjump -l find-providers -d 'Search for provider packages when enumerating reverse dependencies'
